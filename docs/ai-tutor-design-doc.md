...
**Session shape — mobile-first micro-sessions.** The product's primary surface is a phone, and the unit of engagement is a 5–15 minute session. This is a design constraint on everything downstream, not a UI detail: every lesson must decompose into micro-units that are individually completable, every interaction must be **interruptible and resumable** (state saved mid-dialogue, mid-exercise), and a session must be able to open cold with zero setup — the orchestrator decides in advance what the next 10 minutes should contain (new material, review, or remediation) so the learner just opens the app and starts. Micro-sessions are also pedagogically aligned: they force distributed practice over massed practice, and they make the daily review habit (§4.7) and the lesson habit the *same* habit. Hands-free contexts are first-class: audio lessons (§4.5) turn a commute, walk, or chore into a viable session, with the paired retrieval checks done by voice or deferred to the next screen session. Longer deep-work sessions (extended projects, long readings) remain supported but are the exception; the design treats them as optional intensity, not the default.

**Two surfaces.** The product splits across a mobile app and a web portal with distinct jobs. The **mobile app is the learning surface**: lessons (including audio playback), quizzes, dialogue, and the daily review queue. The **web portal is the command center**: viewing and editing the living syllabus, the concept-map mastery dashboard, inspecting and correcting assessment items and the learner model, and the deep-work project workspace. The split follows the interaction grain — learning is many short touches, oversight is occasional long looks — and it keeps the mobile app radically simple: open, learn, close.
...
2. **Generated lessons (text or audio) + exercises.** A personalized expository lesson (tailored to the profile: examples from the learner's profession, building on what they know) followed by retrieval exercises spanning recognition → recall → application. Best for foundational/declarative material and learner-paced study. On mobile, text lessons are authored as screen-sized segments with retrieval checks interleaved every few segments — never a wall of text with a quiz at the end. **Audio is a first-class rendering:** the same lesson script rendered through TTS as a podcast-style micro-episode, written for the ear (spoken register, signposted structure, recaps) rather than read-aloud prose. The design guardrail is that listening alone is passive — the illusion-of-mastery risk (§3.8) is highest here — so every audio episode is bracketed by retrieval: an orienting pre-question before it, and a mandatory quiz after it (immediately, by voice, or at the start of the next session). An episode with no attached retrieval event produces no mastery evidence, and the tracer treats it accordingly.
...
```
┌──────  Clients: mobile app = learning surface · web portal = command center (§4.1)  ─────────┐
│  Mobile: lessons (text/audio) · quizzes · dialogue · review queue                            │
│  Web: syllabus editor · concept-map mastery dashboard · item/model inspection · projects     │
└──────────────────────────────────────────┬───────────────────────────────────────────────────┘
                                           │  API
┌──────────────────────────────────────────▼───────────────────────────────────────────────────┐
│                                    Orchestrator                                              │
...
Mobile-first adds a second job for the same infrastructure: **session pre-composition**. Because sessions are short and opened opportunistically (a spare ten minutes in a queue), the scheduler pre-assembles the next session's contents — due reviews, the next micro-lesson, any pending remediation — ahead of time, so opening the app costs zero planning latency. Audio makes pre-composition mandatory rather than nice-to-have: TTS rendering is an asynchronous pipeline (lesson script → TTS → MP3 artifact → device), so episodes are generated and cached ahead of the session that needs them, ready for offline playback. Mid-session state (dialogue position, audio playback position, partial exercise answers) is persisted continuously so an interrupted session resumes exactly where it stopped.
...
**Phase 2 — depth:** applied projects with rubric grading (multi-session arcs), curated external content, periodic holistic retention checks with remediation, calibration view, profile maintenance loop, eval suite, and the web command center matured into a full mastery dashboard (concept map, item/model inspection, project workspace) rather than just plan viewing and question editing.
...
| A5 | Modality richness | ≥3 of: Socratic dialogue, generated lessons (text/audio)+exercises, rubric-graded projects, curated external content — with principled selection. Audio lessons always paired with retrieval; passive listening earns no mastery credit | 2 |
...
| A21 | Micro-session & resumability design | Mobile-first 5–15 min sessions; lessons decompose into completable micro-units; mid-session state persisted; next session pre-composed (incl. pre-rendered audio) | 2 |
| A22 | Surface split discipline | Mobile app = simple learning surface; web portal = command center (syllabus editing, mastery dashboard, item/model inspection) — mastery oversight actually lives somewhere | 1 |
...
One concrete way to build §5, purely illustrative; the reference design is the contract, not these choices. Mobile client as a PWA or React Native app — chat-centric lesson surface with an audio player (background/lock-screen playback, offline-cached MP3s), review queue as the home screen; push notifications for review nudges; local persistence of mid-session state for interrupt/resume · web portal (React/Next.js) for the syllabus editor, concept-map mastery dashboard, and item/model inspection · a TTS service (provider-agnostic, e.g. cloud neural TTS) invoked by the lesson pipeline, with rendered audio stored in object storage and cached to device · API + orchestrator in a typed server runtime (TypeScript/Node or Python/FastAPI) · Postgres for profiles/courses/graphs/evidence log (append-only table + materialized knowledge-state view) · a job runner (e.g., Temporal, or cron + queue) for the scheduler · LLM access through a thin provider-agnostic layer with structured-output validation (JSON Schema), strongest model tier for planner/tutor/grader, small model tier for review-item generation · FSRS library for memory scheduling · trace logging of all LLM calls to an observability store with an offline eval harness (persona scripts + golden grading sets) run in CI.
